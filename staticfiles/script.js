...
async function handleSearch() {
    ...
    try {
        ...
        // Check for SAP-specific errors
        if (data.sap_error) {
            if (data.sap_status === 'session_expired') {
                showToast('⚠️ SAP Session Expired! Please update the session cookie in sap_config.py', 'warning');
                console.error('SAP Session Cookie expired. Update it in sap_config.py');
            } else {
                showToast(`SAP Warning: ${data.sap_error}`, 'warning');
            }
        }
